...
class BiModalTrainer(object):
    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        for i, data in enumerate(data_loader):
            ...
            #acc_avg = torch.clamp((1/5) * (0.2 - torch.abs(outputs.cpu() - labels.cpu())), min=0).mean()
            # acc_avg = (1/5) * (0.2 - torch.abs(outputs.cpu() - labels.cpu())).clip(min=0).mean()
    ...
    def valid(self, data_loader, model, loss_f, epoch_idx):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(data_loader):
                ...
                #ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(outputs.cpu().detach() - labels.cpu().detach())), min=0).mean(dim=0)
    ...
    def test(self, data_loader, model):
        ...
        with torch.no_grad():
            ...
            for data in tqdm(data_loader):
                ...
                #ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(outputs - labels)), min=0).mean(dim=0)
                mse_ls.append(mse)
    ...
    def full_test(self, data_set, model):
        ...
        #ocean_acc = torch.clamp((1/5) * (0.2 - torch.abs(all_out - all_label)), min=0).mean(0).numpy()
...
class PersEmoTrainer(BiModalTrainer):
    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        for i, data in enumerate(data_loader):
            ...
            #acc_avg = torch.clamp((1/5) * (0.2 - torch.abs(p_score.cpu() - p_labels.cpu()), min=0).mean(dim=0))
            #acc_avg = torch.clamp((1/5) * (0.2 - torch.abs(p_score.cpu() - p_labels.cpu())), min=0).mean(dim=0)
    ...
    def valid(self, data_loader, model, loss_f, epoch_idx):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(data_loader):
                ...
                #ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(p_score.cpu().detach()-p_labels.cpu().detach()), min=0).mean(dim=0))
                #ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(p_score.cpu().detach() - p_labels.cpu().detach())), min=0).mean(dim=0)
    ...
    def test(self, data_loader, model):
        ...
        with torch.no_grad():
            ...
            for data in tqdm(data_loader):
                ...
                # ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(p_score - p_labels), min=0).mean(dim=0))
                #ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(p_score - p_labels)), min=0).mean(dim=0)
