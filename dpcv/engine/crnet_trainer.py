    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        for i, data in enumerate(data_loader):
            ...
            if model.train_guider:
                ...
                acc_avg = torch.clamp((1/5) * (0.2 - torch.abs(reg_pred.cpu() - reg_label.cpu())), min=0).mean()
            ...
            if i % self.cfg.LOG_INTERVAL == self.cfg.LOG_INTERVAL - 1:
                if model.train_guider:
                    ...
                    acc = torch.clamp((1/5) * (0.2 - torch.abs(reg_pred - reg_label)), min=0).mean(dim=0)
        ...
        if not model.train_guider:
            with torch.no_grad():
                ...
                for i, data in enumerate(data_loader):
                    ...
                    ocean_acc_batch = torch.clamp((1/5) * (0.2 - torch.abs(reg_pred.cpu() - reg_label.cpu())), min=0).mean(dim=0)
    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        for i, data in enumerate(data_loader):
            ...
            if not model.train_regressor:
                ...
                # acc_avg = (1 - torch.abs(reg_pred.cpu() - reg_label.cpu())).mean().clip(min=0)
                acc_avg = torch.clamp((1/5) * (0.2 - torch.abs(reg_pred.cpu() - reg_label.cpu())), min=0).mean()
                # acc_avg = acc_avg.detach().numpy() 不是我改的
