...
def swin_config():
    ...
    _C.MODEL.PRETRAIN = True  # Add this line
...
def get_swin_transformer_model(cfg=None):
    ...
    # Check for pretrained weight loading
    if config.MODEL.PRETRAIN and config.MODEL.WEIGHT_PATH:
        checkpoint = torch.load(config.MODEL.WEIGHT_PATH, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        model.load_state_dict(checkpoint, strict=False)  # Allow partial load if necessary
        print("Pretrained weights loaded from:", config.MODEL.WEIGHT_PATH)
