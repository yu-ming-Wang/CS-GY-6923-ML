...
class ExpRunner:
    ...
    def __init__(self, cfg, feature_extract=None):
        ...
        self.model = self.build_model()
        # In exp_runner.py, add this in the __init__ method of ExpRunner, right after self.model is defined
        # if self.cfg.MODEL.WEIGHT_PATH:
        # # Log loading checkpoint
        #     self.logger.info(f"Loading pre-trained weights from {self.cfg.MODEL.WEIGHT_PATH}")
        #     # Load the weights using load_checkpoint function, with strict=False to allow flexibility in keys
        #     # load_checkpoint(self.model, self.cfg.MODEL.WEIGHT_PATH, strict=False, logger=self.logger)
        #     CheckpointLoader.load_checkpoint(self.model, self.cfg.MODEL.WEIGHT_PATH,  # Path to the weight file
        #     strict=False, logger=self.logger)
        print('\n')
    ...
    def build_model(self):
        ...
        # Load pretrained weights if WEIGHT_PATH is specified in the configuration
        weight_path = self.cfg.MODEL.get("WEIGHT_PATH", "")
        if weight_path:
            self.logger.info(f"Loading pre-trained weights from {weight_path}")
            state_dict = torch.load(weight_path, map_location="cuda" if torch.cuda.is_available() else "cpu")

            # Check if 'model' key exists, otherwise use the state_dict as-is
            if 'model' in state_dict:
                state_dict = state_dict['model']

            # Filter out incompatible keys (like classifier weights) with size mismatches
            ...
            self.logger.info("Pre-trained weights loaded successfully with shape-matching only")
        else:
            self.logger.info("No pre-trained weights specified; using random initialization.")
    ...
    def test(self, weight=None):
        ...
        # Record and visualize predictions vs true labels
        self.collector.record_predictions(dataset_output, dataset_label)
        self.collector.draw_predictions(log_dir=self.log_dir)
